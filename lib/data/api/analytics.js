export async function getMatomoActions() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_MATOMO_URL}?method=Actions.get&idSite=${process.env.NEXT_PUBLIC_MATOMO_SITE_ID}&period=year&date=2020-09-20,today&module=API&format=JSON&token_auth=${process.env.NEXT_PUBLIC_MATOMO_API_KEY}`)
...
export async function getMatomoMostViewsPages() {
const getViews = encodeURI(`${process.env.NEXT_PUBLIC_MATOMO_URL}?method=Actions.getPageUrls&idSite=${process.env.NEXT_PUBLIC_MATOMO_SITE_ID}&period=&date=previous50&module=API&format=JSON&token_auth=${process.env.NEXT_PUBLIC_MATOMO_API_KEY}`)
const matomoDataLastViews = await fetch(getViews)
const lastViews = await matomoDataLastViews.json()
if (lastViews.errors) {
  console.error(lastViews.errors)
