...
export async function getMatomoActions() {
    const res = await fetch(
        ...
        }&period=range&date=2018-02-01,${new Date().toISOString().slice(0,10)}&module=API&format=JSON&token_auth=${
...
export async function getMatomoLiveCounter() {
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_MATOMO_URL}?method=Live.getCounters&idSite=${
            process.env.NEXT_PUBLIC_MATOMO_SITE_ID
        }&lastMinutes=30&module=API&format=JSON&token_auth=${
            process.env.NEXT_PUBLIC_MATOMO_API_KEY
        }`
    )
...
export async function getPageUrl(url) {
    const res = await fetch(
        `${
            ...
            process.env.NEXT_PUBLIC_MATOMO_SITE_ID
        }&period=range&date=2018-02-01,${new Date().toISOString().slice(0,10)}&format=JSON&token_auth=${
            process.env.NEXT_PUBLIC_MATOMO_API_KEY
        }&pageUrl=${url}`
    )
