...
import { getComments } from "src/data/external/cms"
    ...
    <Comments>
      ...
      {comments.map((comment, i) => (
        <Comment
          ...
          className="p-comment h-cite comment"
        >
          <CommentAuthor className="p-author h-card author">
            <CommentAuthorName className="p-name u-url">
              {comment.name}
            </CommentAuthorName>
          ...
          <CommentContent className="e-content comment-content">
...
const Comment = styled.li`
  ...
  border-bottom: ${(props) =>
    props.authorName == "Max" ? "2px solid var(--thirdy-color)" : "none"};
